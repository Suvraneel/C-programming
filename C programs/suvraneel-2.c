...
Compiler used : Gcc 6.3 (https://www.codechef.com/ide)
*******************************************************************************/
...
{   
    ...
    //dynamic allocation
    a = (char *)malloc(1+m*sizeof(char));         		//+1 so that final product carry isn't dropped
    b = (char *)malloc(1+n*sizeof(char));
    digit = (int *)malloc(1+(m+n)*sizeof(int));			//+1 so that final sum carry isn't dropped
    ...
    for(j=n; j>=0; j--){
        ...
        for(i=m; i>=0; i--){
            ...
            carry = ((a[i]-'0') * (b[j]-'0') + carry)/10 ;          		//stores product carry
            int sumcarry = digit[k]/10;                            		//stores sum carry
            digit[k] = digit[k]%10;
            k++;
    ...
    int k;
    for(k=m+n-1; k>= 0; k--)            			//printing output from integer array in reverse order
        printf("%d", digit[k]);
